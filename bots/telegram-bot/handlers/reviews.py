...
def reviews_menu_kb(page: int = 0, total: int = 0, channel_url: str = ""):
    buttons = []
    nav = []
    if page > 0:
    ...
    if channel_url:
        buttons.append([
            InlineKeyboardButton(text="📢 Канал с отзывами", url=channel_url)
        ])
    buttons.append([InlineKeyboardButton(text="✍️ Написать отзыв", callback_data="add_review")])
...
async def process_review_text(message: Message, state: FSMContext):
    ...
    await message.answer("Оцените покупку:", reply_markup=rating_kb())
...
async def process_review_photo(message: Message, state: FSMContext):
    ...
    await db.add_review(user.id, user.username or "", data["text"], data["rating"], photo_file_id)
